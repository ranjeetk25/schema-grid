...
import { cursorFromRow } from "../../../src/pagination/keyset";
...
describe("buildQuery", () => {
  it("§8 filter with offset paging", () => {
    ...
    expect(built.fingerprint).toBe(queryFingerprint(SECTION_8));
  });
  ...
  it("an offset-mode cursor resumes at its offset", () => {
    ...
    const cursor = encodeCursor({ v: 1, mode: "offset", fp: queryFingerprint(query), offset: 30 });
  ...
  it("count omits the keyset predicate", () => {
    ...
    const cursor = encodeCursor({ v: 1, mode: "keyset", fp: queryFingerprint(base), keys: ["Z"], id: "r1" });
  ...
  it("a cursor from a different query throws CursorError", () => {
    ...
    const cursor = encodeCursor({ v: 1, mode: "keyset", fp: queryFingerprint(a), keys: ["x"], id: "r1" });
  ...
  it("a keyset cursor without an id throws CursorError", () => {
    ...
    const cursor = encodeCursor({ v: 1, mode: "keyset", fp: queryFingerprint(a), keys: [] });
