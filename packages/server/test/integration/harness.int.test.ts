...
import { FIXTURE_GRID_ID, FIXTURE_NOW, admissionsRows, admissionsSchema } from "../fixtures/admissions";
...
describeMysql("mysql integration harness", () => {
  ...
  beforeAll(async () => {
    ...
    await setupGrid(mysql.db, admissionsSchema, admissionsRows, { gridId: FIXTURE_GRID_ID, now: FIXTURE_NOW });
  }, 180_000);
  afterAll(async () => {
    await mysql?.stop();
  });
  ...
  it("applies the DDL and seeds the fixture", async () => {
    ...
    expect(Number(rows[0]?.n)).toBe(admissionsRows.length);
  });
