...
const errs = (n: number) => Array.from({ length: n }, (_, i) => ({ message: `bad ${i}` }));

describe("formatClipboardReport", () => {
  it("full success is green", () => {
    expect(formatClipboardReport({ pastedCells: 40, skippedReadOnly: 0, errors: [] })).toEqual({
  ...
  it("partial is yellow with breakdown", () => {
    const r = formatClipboardReport({ pastedCells: 40, skippedReadOnly: 1, errors: errs(2) });
    expect(r.message).toBe("Pasted 40 cells, 3 skipped (2 invalid, 1 read-only)");
    expect(r.color).toBe("yellow");
  });
  it("total failure is red", () => {
    const r = formatClipboardReport({ pastedCells: 0, skippedReadOnly: 0, errors: errs(3) });
    expect(r.message).toBe("Pasted 0 cells, 3 skipped (3 invalid)");
    expect(r.color).toBe("red");
    expect(r.title).toBe("Paste failed");
  });
  it("singular cell", () => {
    expect(formatClipboardReport({ pastedCells: 1, skippedReadOnly: 0, errors: [] }).message).toBe("Pasted 1 cell");
  });
});
...
describe("notifyClipboardReport", () => {
  it("shows through the lazily loaded notifications module", async () => {
    ...
    const result = await notifyClipboardReport(
      { pastedCells: 2, skippedReadOnly: 0, errors: [] },
  ...
  it("returns unavailable when the module has no notifications export", async () => {
    const result = await notifyClipboardReport({ pastedCells: 2, skippedReadOnly: 0, errors: [] }, { loader: async () => ({}) });
    expect(result).toBe("unavailable");
  });
});
