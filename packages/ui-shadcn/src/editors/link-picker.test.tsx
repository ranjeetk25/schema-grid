...
function setup(overrides: Partial<Parameters<typeof LinkPickerEditor>[0]> = {}, allowMultiple = false) {
  ...
  const utils = renderUi(<LinkPickerEditor {...props} />);
...
describe("LinkPickerEditor", () => {
  ...
  it("calls lookup with an empty search on open, then with the typed search", async () => {
    ...
    await user.type(screen.getByRole("combobox"), "Lead #2");
  ...
  it("keyboard: Enter picks the highlighted record and never reaches the grid", async () => {
  ...
  it("multi mode keyboard: Enter picks, then Enter on the empty search commits", async () => {
    const { user, props } = setup({}, true);
    await screen.findByText("Lead #1");
    ...
    expect(props.onCommit).not.toHaveBeenCalled();
    await user.keyboard("{Enter}");
    expect(props.onCommit).toHaveBeenCalledWith([LEAD_1]);
  });
