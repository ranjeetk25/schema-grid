...
const colourGroup = (index = 0) => screen.getAllByRole("radiogroup", { name: "Option colour" })[index] as HTMLElement;
...
describe("ZodForm", () => {
  ...
  it("adding 'Paid' and choosing green emits the option", async () => {
    ...
    const { user } = renderUi(<Harness schema={selectSchema()} onChange={onChange} />);
    ...
    await user.click(within(colourGroup()).getByRole("radio", { name: "green" }));
    expect(lastCall(onChange)).toStrictEqual({ options: [{ id: "paid", label: "Paid", color: "green" }] });
    expect(within(colourGroup()).getByRole("radio", { name: "green" })).toHaveAttribute("aria-checked", "true");
  });
  ...
  it("omits color when none was chosen, and when the schema has no color field", async () => {
    ...
    const r2 = renderUi(<Harness schema={noColor} onChange={onChange2} />);
    await r2.user.click(within(r2.container).getByRole("button", { name: "Add option" }));
    expect(within(r2.container).queryByRole("radiogroup", { name: "Option colour" })).toBeNull();
  });

  it("shows the colour swatches upfront for every option, the current tone checked", () => {
    renderUi(
      <ZodForm
        schema={selectSchema()}
        value={{
    ...
    expect(screen.getAllByRole("radiogroup", { name: "Option colour" })).toHaveLength(2);
    expect(within(colourGroup(0)).getByRole("radio", { name: "red" })).toHaveAttribute("aria-checked", "true");
    expect(within(colourGroup(1)).queryByRole("radio", { checked: true })).toBeNull();
  });

  it("moves between swatches with the arrow keys", async () => {
    const onChange = vi.fn();
    const { user } = renderUi(
      <Harness schema={selectSchema()} initial={{ options: [{ id: "a", label: "A", color: "red" }] }} onChange={onChange} />,
    );
    within(colourGroup()).getByRole("radio", { name: "red" }).focus();
    await user.keyboard("{ArrowRight}");
    expect(lastCall(onChange)).toStrictEqual({ options: [{ id: "a", label: "A", color: "orange" }] });
    expect(within(colourGroup()).getByRole("radio", { name: "orange" })).toHaveFocus();
  });
  ...
  it("reorders and removes options", async () => {
    ...
    const { user } = renderUi(<Harness schema={selectSchema()} initial={initial} onChange={onChange} />);
  ...
  it("renders a boolean as a switch", async () => {
    const onChange = vi.fn();
    const { user } = renderUi(<ZodForm schema={z.object({ allowMultiple: z.boolean() })} value={{}} onChange={onChange} />);
  ...
  it("renders strings, numbers and enums; marks optional fields", async () => {
    ...
    const { user } = renderUi(<Harness schema={schema} onChange={onChange} />);
  ...
  it("renders an unsupported union as JSON; invalid JSON shows an error and does not emit", async () => {
    ...
    renderUi(<ZodForm schema={schema} value={{ mixed: 1 }} onChange={onChange} />);
