...
function BadgeRenderer({ value, config }: UiRendererProps<string>) {
  ...
  return opt ? <span data-testid="preview-badge">{opt.label}</span> : null;
}
...
describe("PreviewStep", () => {
  it("renders the sample through the type renderer, and edits update it", async () => {
    const { user } = renderUi(<PreviewStep draft={selectDraft()} registry={registry} uiRegistry={uiRegistry} />);
