...
const OK = { pastedCells: 2, skippedReadOnly: 0, conflicts: 0, errors: [] };
...
describe("notifyClipboardReport", () => {
  ...
  it("routes severity: partial → warning, failure → error, nothing → plain toast", async () => {
    ...
    await notifyClipboardReport({ pastedCells: 4, skippedReadOnly: 1, conflicts: 0, errors: [] }, { loader });
    ...
    await notifyClipboardReport({ pastedCells: 0, skippedReadOnly: 0, conflicts: 0, errors: errs(1) }, { loader });
    ...
    await notifyClipboardReport({ pastedCells: 0, skippedReadOnly: 0, conflicts: 0, errors: [] }, { loader });
