...
function setup(
  ...
  const utils = renderUi(<ImportWizard {...base} />);
...
describe("ImportWizard preview + run", () => {
  it("marks invalid cells and shows the message in a tooltip", async () => {
    ...
    expect(await screen.findByRole("tooltip")).toHaveTextContent("Invalid date");
  ...
  it("runs end to end with the real io: parse, auto-map, validate, reject unknown options, commit", async () => {
    ...
    expect(await screen.findByRole("combobox", { name: "Map Payment Status" })).toHaveTextContent("Payment status");
    expect(screen.getByRole("combobox", { name: "Map Notes" })).toHaveTextContent("Notes");
    expect(screen.getByRole("combobox", { name: "Map Website" })).toHaveTextContent("Website");
    await user.click(screen.getByRole("button", { name: "Continue" }));
    ...
    expect(await screen.findByRole("tooltip")).toHaveTextContent('Unknown option "Refunded"');
...
describe("ImportWizard run step chrome", () => {
  it("Done closes the wizard after the import finishes", async () => {
    const onClose = vi.fn();
    const utils = setup({ props: { onClose } });
    await toPreview(utils);
    await utils.user.click(screen.getByRole("button", { name: "Start import" }));
    await screen.findByText("Waiting for import to start…");
    utils.rerenderWith({ onClose, job: { state: "done", processed: 2, total: 2, errorCount: 0 } });
    expect(screen.getByText(/Done · 2 of 2 rows/)).toBeInTheDocument();
    await utils.user.click(screen.getByRole("button", { name: "Done" }));
    expect(onClose).toHaveBeenCalledTimes(1);
  });

  it("summary tiles expose readable counts", async () => {
    const utils = setup();
    await toPreview(utils);
    expect(screen.getByText("2 valid rows")).toBeInTheDocument();
    expect(screen.getByText("0 invalid rows")).toBeInTheDocument();
    expect(screen.getByText("0 unknown values")).toBeInTheDocument();
  });
});
